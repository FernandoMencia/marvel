...
async function fetchMarvelCharacters(name) {
  ...
  try {
    ...
    if (error.response) {
      ...
      if (statusCode === 409) {
        //  diferentes tipos de errores de la API de Marvel
        switch (errorMessage) {
          case 'Limit greater than 100.':
            return res.status(409).json({ error: 'El límite debe ser igual o menor que 100.' });
          case 'Limit invalid or below 1.':
            return res.status(409).json({ error: 'El límite debe ser un número válido y mayor que 0.' });
          case 'Invalid or unrecognized parameter.':
            return res.status(409).json({ error: 'El parámetro proporcionado es inválido o no reconocido.' });
          default:
            return res.status(409).json({ error: 'Error desconocido al procesar la solicitud.' });
        }
      }
    }
